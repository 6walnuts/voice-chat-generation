...
python3 recorder/server.py          # 采集：照稿朗读（A 批优先）+ 自由说话
python3 scripts/export_dataset.py   # 导出 dataset/gptsovits.list 并做质量体检
```

体检报告里提示削波/过短/过小声的条目，建议回录音页重录（会自动覆盖），
然后重新导出一次。两类数据（朗读句 A01…F20 与自由说话段 S001…）会一起导出、
一起参与训练——朗读保发音覆盖，自由说话保自然语气，混合训练效果最好。

> 我们的数据要么「先有文本、照着念」，要么「切分后人工逐段校对」，
> 文本都是精确对齐的，因此 **GPT-SoVITS WebUI 里「0-前置数据集获取工具」
> （切割、ASR、打标）整个跳过**——没有未经校对的语音识别错误混进训练集。
...
2. 推理页里：
   - **参考音频**：上传一条你录得最满意的 3~10 秒录音，并把它的文本填进
     「参考音频的文本」（在 `dataset/meta.jsonl` 里能查到）。
     参考音频的语气会直接影响合成语气——**想要日常聊天感，选自由说话的
     S 段（如 `dataset/wavs/S003.wav`）；想要清晰播报感，选朗读句
     （如 `dataset/wavs/A24.wav`）**；想要什么情绪就选哪条。
